<script lang="ts">
	...
	export function fixProvincePositions() {
		...
		console.log(`✅ ${fixed} provinces reset to original position`);
		return fixed;
	}

	// Update background colors in real-time
	export function updateBackgroundColor(type: string, hexColor: string) {
		console.log(`🎨 updateBackgroundColor: type=${type}, color=${hexColor}`);
		if (!scene) {
		...
		if (type === 'primary') {
			...
			console.log(`✅ Scene background and fog updated to ${hexColor}`);
		}
	}

	// Update region colors in real-time
	export function updateRegionColor(regionKey: string, hexColor: string) {
	...
	export function updateWaterColor(waterKey: string, hexColor: string) {
		const color = parseInt(hexColor.replace('#', ''), 16);
		...
		waterBodies.forEach(w => {
			if (w.id === actualId) {
				...
				console.log(`✅ رنگ ${waterKey} تغییر کرد`);
			}
		});
	}
	...
	export function applyConfigChanges(path, value) {
		console.log(`🔧 applyConfigChanges called: path=${path}, value=${value}`);
		if (!scene) {
			console.warn(`⚠️ Scene not initialized yet`);
			return;
		}
		...
		if (path === 'background.vignette') {
			console.log(`🌫️ Updating vignette to ${value}`);
			if (typeof document !== 'undefined') {
				...
				console.log(`✅ Vignette updated to ${value}`);
			}
		}
		// Bloom settings
	...
	onMount(async () => {
		...
		if (!svgData) {
			...
			try {
				...
				console.log('✅ SVG loaded, length:', svgData.length);
			} catch (error) {
	...
	function loadMapFromSVG(svgString) {
		...
		console.log(`📍 Found ${pathElements.length} province paths`);
		...
		pathElements.forEach(pathEl => {
			...
			if (provinceColors[className] && pathData) {
				console.log(`✅ Loading province: ${className}`);
				createProvinceFromPath(className, pathData, provinceColors[className]);
				loadedCount++;
			} else {
				console.warn(`⚠️ Skipping province: ${className} (no color or path data)`);
			}
		});
		
		console.log(`✨ Loaded ${loadedCount} provinces successfully`);
	...
	function loadWaterBodies(doc) {
		...
		if (caspianPath) {
			...
			if (pathData) {
				console.log('✅ Loading Caspian Sea');
		...
		console.log('✅ Loading South Waters (Persian Gulf + Oman Sea)');
		...
		mainLakes.forEach(lakeName => {
			...
			if (lakePath) {
				...
				if (pathData) {
					console.log(`✅ Loading lake: ${lakeName}`);
					createSeaFromPath(lakeName, pathData);
				}
			}
		});
		
		console.log(`🌊 Loaded ${waterBodies.length} water bodies`);
	}
	...
	function parseSVGPath(pathString) {
		...
		if (!commands) {
			console.warn('No commands found in path');
			return null;
		}
		...
		try {
			...
			console.error('Error parsing path:', e, pathString.substring(0, 100));
			return null;
		}
	}
